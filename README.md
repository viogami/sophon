...
当前版本适合用 1,000 条角色数据验证完整链路，不包含剧情事件、关系图谱或增量 RAG
投影。

## 数据边界

`data/` 是私有输入数据目录，已被 `.gitignore` 排除，不会提交到仓库或打入 Python
wheel。运行前需要由部署者自行将数据放置为以下结构：
...
Sophon 不负责抓取或公开分发这些数据。后续定时任务只应在私有数据更新完成后调用同步脚本。
